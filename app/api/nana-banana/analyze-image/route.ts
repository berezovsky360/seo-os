...
export async function POST(request: NextRequest) {
  try {
    ...
    const { image_base64, article_title, focus_keyword } = body
    if (!image_base64 || !article_title) {
      return NextResponse.json({ error: 'Missing image_base64 or article_title' }, { status: 400 })
    }
    ...
    const result = await module.executeAction('analyze_image_seo', { image_base64, article_title, focus_keyword }, context)

    return NextResponse.json(result)
  } catch (error) {
    console.error('[API] nana-banana/analyze-image error:', error)
