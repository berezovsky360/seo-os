...
export async function POST(request: NextRequest) {
  try {
    ...
    const { site_id, post_id, aspect_ratio } = body
    if (!site_id || !post_id) {
      return NextResponse.json({ error: 'Missing site_id or post_id' }, { status: 400 })
    }
    ...
    const result = await module.executeAction('run_full_pipeline', { site_id, post_id, aspect_ratio }, context)

    return NextResponse.json(result)
  } catch (error) {
    console.error('[API] nana-banana/pipeline error:', error)
