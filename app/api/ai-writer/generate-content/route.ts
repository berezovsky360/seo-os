...
export async function POST(request: NextRequest) {
  try {
    ...
    const { topic, persona_id, site_id, writing_style, model: requestModel } = body
    if (!topic) {
    ...
    const result = await module.executeAction('generate_content', { topic, persona_id, site_id, writing_style }, context)
    ...
    if (result.usage) {
      ...
      await serviceClient.from('ai_usage_log').insert({
        ...
        estimated_cost,
        metadata: { topic, site_id, writing_style },
      })
    ...
    console.error('[API] ai-writer/generate-content error:', error)
