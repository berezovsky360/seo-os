...
export async function POST(request: NextRequest) {
  try {
    ...
    const module = new PersonasModule()
    const result = await module.executeAction('process_document', { document_id }, context)

    return NextResponse.json(result)
  } catch (error) {
    console.error('[API] personas/process-document error:', error)
